...
export default function LinearSearchBar() {
  ...
  return (
    <div className="flex flex-col items-center space-y-2 w-full ">
      <div className="flex items-end justify-center flex-wrap gap-1 md:p-20 p-5 w-full">
        {bars.map((val, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center">
              {IsElementFound && searchIndex === i && (
                <div className="text-xs md:text-sm text-green-400 mb-1">✅ Found!</div>
              )}
              ...
              <div className="text-xs md:text-sm text-cyan-300 mt-1">{i}</div>
            </div>
          );
        })}
      </div>
      ...
      {searchInput === '' && (
        <div className="text-white font-semibold mt-2 mb-4 text-center w-full text-sm md:text-base">
